...
| **11 — Visual & UX Overhaul** | ▶ **in planning (pulled ahead of 9–10)** | Cartographer's re-skin (tokens/type/terrain art), map-chrome polish, **faction shaded/hatched territory + legend→detail**, **radial = the single hex-action surface**, sidebar → **overview** dashboard, dungeon restyle, responsive/touch, a11y. **Supersedes** 10.6 (themes) + 10.8 (art) + touch half of 10.2. Presentation-only — **no schema bump**. See [phase-11-visual-overhaul.md](docs/plans/phase-11-visual-overhaul.md) |

Phases 0→1→2→3→4→5 are a hard chain; 6/9 need only the map + POIs; 7 and 8 are done. **Remaining work
order: 11 → 9 → 10** — the Visual & UX Overhaul (pulled ahead at the user's request), then the
