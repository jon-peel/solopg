...
(8.1–8.20)** — Arc A (Travel & Party Movement, 8.1–8.6) + Arc B (Factions, 8.7–8.10) + the
**expansion arc (8.13–8.20)**, where faction turns are a **contested spatial engine** — every faction
grows a **sphere of influence** around a **seat** (its HQ), at a **per-archetype expansion chance**,
seizing rivals' hexes strength-weighted; a fallen seat **relocates or dissolves**, and factions also
**recede and disband naturally** over time (8.20) — all **played as subtext, not hooks**. Lair-bound **lords** (necromancer/lich/vampire/dragon/hag) infuse their own
...
- **8.14 (built, then removed).** The region **"something is stirring"** hook — a broad tract-level
  escalation signal (`regionHeat`, `rollRegionStir`, `buildRegionHook`, the `region-omen` table) — was
  the last faction-driven lead, but it went the same way as the rest: **removed** so factions emit no
  hooks at all. (The region *naming* engine, `computeRegions`, stays — it still labels tracts on the map.)
...
**Phase 8.20 — factions decline and disband on their own (organic ebb).** A faction with no rivals used
to grow forever and never die. Now, on a turn where it **didn't grow** (gate failed or no room), a
rank-and-file faction has a small chance (`CONTRACTION_CHANCE` = 0.15) to **recede** (shed its outermost
SOI hex — the seat is never lost this way) and, when worn down to just its seat, to **disband** (a
natural death — `eliminated` with no finisher; the log says *"fades into history"* vs *"is destroyed"*).
Growth is rolled **first** and is likelier, so the map trends up with an ebb: thriving factions (still
finding room) rarely decline; stagnant/boxed-in ones fade. **Lair-bound lords are exempt** — a lich or
dragon holds its lair until something kills it. New event `recede`; no schema change. Suite green at
**437 `node --test`**.
...
| **9 — Small oracles & the living world** | ✅ **built (9.1–9.9)** | The app is a **referee's oracle, not a rulebook**. **Generative** oracles (Oracle tab): **9.1** surface · **9.2** Yes/No fate (odds ladder + six-outcome …and/…but + random-event flag) · **9.3** Meaning · **9.4** Complication · **9.5** Settlement situation (context-aware, faction-coloured) · **9.6** Tavern/shop. **Trigger-and-prompt** (GM rolls their own tables): **9.7** wilderness-encounter check → **stars on the travel route** (hover shows terrain); **9.8** treasure = a **B/X Treasure Type letter** in dungeons/towers (replaces rolled gp; `DUNGEON_BUILD 22`/`TOWER_BUILD 3`). **9.9** automatic faction **emergence** — a **pressure model** (external ∝ open frontier, internal **rebellions** ∝ dominance, **lords awaken** rarely at eligible lairs), party-biased, narrated as subtext. **Out:** weather / NPC / reaction-morale. See [phase-9-oracles.md](docs/plans/phase-9-oracles.md). *Follow-up UX in Phase 12.* |
...
| **12 — Living-world surfacing & UX polish** | ▶ **planned** | Surfaces Phase 9's living world on-screen (it currently narrates to the **console** only) + a panel polish pass. **12.1** Factions off the tab bar → clickable legend + faction popup (fixes tab overflow) · **12.2** persisted **Chronicle** + a near-party bottom **ticker** (distant events log silently) · **12.3** encounter-star **hover** · **12.4** persistent **settlement taverns** (saved/exported, close/add) · **12.5** treasure-type **badge** · **12.6** legend keys + **lord marker** · **12.7** visual polish (Oracle tab, dungeon room, tile/settlement detail). Full hand-off detail in [phase-12-ux.md](docs/plans/phase-12-ux.md). |
...
| **15 — Monasteries** | ✅ **done (Steps 1–9)** | A rare religious **settlement variant** (`settlement.kind = "monastery"`, mutually exclusive with `keep`, terrain-biased & rarer): culture-aware **name + dedication** (reuses `RACE_DEITIES`), **size → self-sufficiency** + race-leaned **industries**, a **library** + a Phase-9 **research** odds-ladder oracle (bigger library skews to "exact"), **relics** ("Keeps: …"), **catacombs** (a lazily-built **"Catacombs"** dungeon reusing the interior generator, depth by size), and a rare **GM/discovery-only secret** — never in the player panel (test-enforced `publicMonasteryFields` sanitizer; universal reveal control so the surface never betrays which houses are tainted). **Bonus:** a rollable **"Ruined abbey"** wilderness dungeon theme + a **Fallen Order** monster family. New `js/gen/monastery.js`; additive, **no migration**; +65 tests. See [phase-15-monasteries.md](docs/plans/phase-15-monasteries.md). |

Phases 0→1→2→3→4→5 are a hard chain; 6/9 need only the map + POIs; 7, 8 and 11 are done. **Remaining
work order: 9 → 10** — the small-oracle catalog, then the Phase 10 backlog, worked in that sequence
rather than opportunistically. (Phase 11, the Visual & UX Overhaul, was pulled ahead at the user's
request and is complete.)
