...
| **6.3** | **Treasure maps** — Map pattern: target **plus a revealed corridor**; a **"Read map" trigger** (available from **any** selected cell, not just towns) that forces a Map hook; "found map" source flavour. | 🔨 built — awaiting manual test |
